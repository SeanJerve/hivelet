...
const OPEN_WRITES = new Map([
  ...
  ['POST /auth/register', 'CLOSED by default since 2026-09-19 - refuses with 403 unless ALLOW_PUBLIC_SIGNUP=true. It was described here as "public by necessity", and it is not: nothing in the interface reaches it, the sign-in screen dropped its sign-up link deliberately, and a profiles row inserted here carries role tenant and account_status active, which puts a stranger straight into the owner Active Tenants list. Still rate limited 5 per 15 minutes behind the gate, so hammering a closed endpoint is throttled too, and a registration that IS allowed now writes TENANT_CREATE as well as AUTH_LOGIN'],
