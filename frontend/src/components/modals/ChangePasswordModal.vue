...
<template>
  ...
  >
    <div
      ...
      @click.self="close"
    >
      <div class="surface-card w-full max-w-md shadow-2xl rounded-2xl p-6 space-y-5 bg-white my-6">
        <!-- Header -->
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="font-display font-black text-base text-foreground">Change Password</h2>
          ...
          <button
            type="button"
            class="rounded-lg p-1.5 hover:bg-muted transition-colors cursor-pointer"
            aria-label="Close"
            @click="close"
          >
        ...
        <form class="space-y-4" @submit.prevent="submit">
          ...
          <div class="space-y-1.5">
            <label for="cp-current" class="text-xs font-extrabold uppercase tracking-widest text-muted-foreground">
              Current password
            </label>
            <input
              id="cp-current"
              ...
              class="w-full rounded-xl border border-border px-3.5 py-2.5 text-sm text-foreground bg-white focus:outline-none focus:ring-2 focus:ring-primary/30"
              :class="currentPasswordError ? 'border-danger' : ''"
              @input="currentPasswordError = ''"
            />
            <p v-if="currentPasswordError" class="text-xs font-bold text-danger">
          ...
          <!-- New -->
          <div class="space-y-1.5">
            <label for="cp-new" class="text-xs font-extrabold uppercase tracking-widest text-muted-foreground">
              New password
            </label>
            <div class="relative">
              <input
                ...
                class="w-full rounded-xl border border-border px-3.5 py-2.5 pr-11 text-sm text-foreground bg-white focus:outline-none focus:ring-2 focus:ring-primary/30"
              />
              <button
                type="button"
                class="absolute right-2 top-1/2 -translate-y-1/2 rounded-lg p-1.5 hover:bg-muted transition-colors cursor-pointer"
                :aria-label="reveal ? 'Hide password' : 'Show password'"
                @click="reveal = !reveal"
              >
                <EyeOff v-if="reveal" class="size-4 text-muted-foreground" />
            ...
            <ul class="space-y-1 pt-1">
              ...
              >
                <Check v-if="rule.met" class="size-3.5 text-primary shrink-0" />
                <span v-else class="size-3.5 rounded-full border border-border shrink-0" />
                <span>{{ rule.label }}</span>
              </li>
            </ul>

            <p v-if="sameAsCurrent" class="text-xs font-bold text-danger">
          ...
          <!-- Confirm -->
          <div class="space-y-1.5">
            <label for="cp-confirm" class="text-xs font-extrabold uppercase tracking-widest text-muted-foreground">
          ...
          <p v-if="formError" class="text-xs font-bold text-danger">{{ formError }}</p>

          <div class="flex items-center justify-end gap-2 pt-1">
            <button
              type="button"
              class="rounded-xl px-4 py-2.5 text-xs font-bold text-foreground hover:bg-muted transition-colors cursor-pointer"
            ...
            <button
              type="submit"
              class="rounded-xl bg-primary px-4 py-2.5 text-xs font-bold text-white transition-opacity cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              :disabled="!canSubmit"
            >
