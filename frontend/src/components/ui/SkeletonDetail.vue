...
<template>
  <div class="w-full" aria-hidden="true">
    ...
    <div class="grid border-t border-border lg:grid-cols-[1fr_26rem]">
      <div class="sd-pulse aspect-[4/3] border-b border-border bg-muted lg:aspect-auto lg:min-h-[30rem] lg:border-b-0 lg:border-r" />

      <div class="px-6 py-10 sm:px-8 sm:py-12 lg:px-10">
        <div class="h-2.5 w-32 sd-pulse bg-muted" />
        <div class="mt-6 h-10 w-48 sd-pulse bg-muted" />
        <div class="mt-4 h-3 w-36 sd-pulse bg-muted" />
        <div class="mt-9 h-8 w-40 sd-pulse bg-muted" />

        <div class="mt-10 border-t border-foreground">
          <div v-for="i in 2" :key="i" class="flex items-center justify-between gap-6 border-b border-border py-4">
            <div class="h-3 w-20 sd-pulse bg-muted" />
            <div class="h-3 w-28 sd-pulse bg-muted" />
          </div>
        </div>

        <div class="mt-12 h-11 w-full sd-pulse bg-muted" />
      </div>
    </div>
    ...
    <div class="mt-16 grid grid-cols-2 gap-x-6 gap-y-8 sm:grid-cols-3 sm:gap-x-8 lg:grid-cols-4 xl:grid-cols-5">
      <div v-for="i in 5" :key="'plate-' + i" class="border-t border-border px-4 pt-5 pb-6">
        <div class="flex items-baseline justify-between gap-3">
          <div class="h-4 w-12 sd-pulse bg-muted" />
          <div class="h-2.5 w-14 sd-pulse bg-muted" />
        </div>
        <div class="mt-4 h-3.5 w-20 sd-pulse bg-muted" />
        <div class="mt-2 h-2.5 w-16 sd-pulse bg-muted" />
      </div>
    </div>
  </div>
</template>
