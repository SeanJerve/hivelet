 ...
 *   3. Every hardcoded money fallback must still equal the live configured rate.
 *      A fallback is only safe while it agrees with the thing it stands in for,
 *      and nothing was checking that — which is exactly how 30 of 33 seeded
 *      prices went stale without a word.
 */
...
const COUNTS_ONLY = new Map([
  ...
  ['fetchWaterRates', 'sets no collection — its fallbacks are covered by rule 3 instead'],
]);
...
// ---------------------------------------------------------------- rule 3 ----
const perHeadFallbacks = [...src.matchAll(/waterRates\.perOccupant\s*\?\?\s*(\d+)/g)].map((m) => Number(m[1]));
const lindaFallbacks = [...src.matchAll(/code === 'LF'\s*\?\s*(\d+)\s*:\s*(\d+)/g)]
  .map((m) => ({ LF: Number(m[1]), LB: Number(m[2]) }));
...
if (!rates || typeof rates.waterRatePerOccupant !== 'number') {
  ...
  const drift = [];
  for (const v of perHeadFallbacks) {
    if (v !== rates.waterRatePerOccupant) {
      drift.push(`per-occupant fallback ${v} vs configured ${rates.waterRatePerOccupant}`);
    }
  }
  const linda = rates.lindaFixedWaterCharges || {};
  for (const f of lindaFallbacks) {
    if (linda.LF !== undefined && f.LF !== Number(linda.LF)) {
      drift.push(`LF fallback ${f.LF} vs configured ${linda.LF}`);
    }
    if (linda.LB !== undefined && f.LB !== Number(linda.LB)) {
      drift.push(`LB fallback ${f.LB} vs configured ${linda.LB}`);
    }
  }
  ...
  check(
    ...
    `${perHeadFallbacks.length} per-occupant and ${lindaFallbacks.length} Linda fallbacks, ` +
      `all equal to the configured ₱${rates.waterRatePerOccupant}/head` +
      (linda.LF !== undefined ? `, LF ₱${linda.LF}, LB ₱${linda.LB}` : ''),
    `a fallback no longer matches what it stands in for: ${drift.join('; ')}. ` +
      'A failed rate fetch would now quietly show the old figure. Update the ' +
      'fallback in systemState.ts to the configured value.'
  );
}
