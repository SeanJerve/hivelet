...
sequenceDiagram
    ...
    Note over API, DB: ATOMIC since migration 018. These three writes run inside settle_verified_payment(), a plpgsql function whose body is one implicit transaction - so they commit together or not at all. Verified by rejecting the ledger row mid-call: the payment stayed Pending Verification and the bill stayed Due. supabase-js still cannot open a transaction, so this pattern is how atomicity is obtained here.
