...
| D-5 | `fifty_percent_share` and `remitted_amount` are computed at `admin.ts:911` and `:1102-1104` but omitted from both INSERTs, so every `monthly_income_records` row currently stores `0.00` | `backend/src/routes/admin.ts:911` and `:1102-1104` vs `:942-958` and `:1112-1129` | Phase 3 |
