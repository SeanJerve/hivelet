...
| 6 | ~~Foreign keys are 17 CASCADE / 4 SET NULL / 0 RESTRICT~~ **FIXED by `005`.** ~~Now 7 RESTRICT / 11 CASCADE / 1 SET NULL / 19 NO ACTION~~ **Recounted live 2026-09-15: 8 RESTRICT / 11 CASCADE / 1 SET NULL / 18 NO ACTION.** The eighth RESTRICT is `clusters_expense_area_fkey`, added by migration `012` after the earlier count was taken. | live catalogue | done |
| 7 | No `BEGIN`/`COMMIT` transaction anywhere in `backend/src` — still true, and permanently so: supabase-js cannot open one. ~~`replace_expense_allocations` (`010`) is the only atomic multi-row write~~ **Stale as of 2026-09-15: there are now four.** `010` expense allocations, `018` payment settlement, `019` expense-entry creation, `020` rate-change history (a trigger). Atomicity is current behaviour, not a design target. What remains Phase 3 is the *rule*: any NEW multi-step write must follow the same database-function pattern rather than chaining awaits | `database/migrations/010, 018, 019, 020` | rule stands |
