...
| ID | Name | Rule | Enforcement locus (verified) | Status |
| --- | --- | --- | --- | --- |
| **BR-009** | Inquiry Conversion | Conversion reuses stored prospect information rather than forcing retyping; conversion does not guarantee tenancy. | Previously recorded as "no prospect-to-onboarding field carry-over is implemented, so the no-retyping clause is unmet". **That is wrong - the carry-over exists.** `InquiriesView.vue:262` routes the prospect's name, phone, email and target unit into the onboarding form, and `TenantManagementView.vue:69` pre-fills them, syncs the advance rent to the unit and opens the modal. **What was genuinely missing, fixed 2026-09-14:** nothing was written back. After onboarding, the lead stayed `Pending` in the inbox indefinitely, and `inquiries.converted_tenant_id` - a column present since the original schema - was set by nothing anywhere, so no record connected a tenancy to the enquiry that produced it. The onboarding flow now marks the inquiry `Converted` and writes the link, validating that the profile exists first so a bad id reads as a message rather than a foreign-key error. It is deliberately non-fatal: the tenant exists either way, and re-running onboarding to correct a lead's status would create a duplicate person. The rule's second clause holds by construction - the status is recorded separately from the tenancy and neither implies the other. | **Enforced** |
...
| ID | Name | Rule | Enforcement locus (verified) | Status |
| --- | --- | --- | --- | --- |
| **BR-010** | Due Date | The monthly due date derives from the move-in date and the billing relationship. | Carrier columns `room_assignments.anniversary_date` and `bills.due_date`; onboarding seeds the anniversary from the move-in date (`backend/src/routes/admin.ts`). The recorded evidence - "the one bill-creating path hardcodes `now + 5 days`" - is **stale**. **Verified 2026-09-14:** there are two bill-creating paths and both derive the date from the tenancy anniversary through `billingService.computeBillPeriod()` - `tenant.ts:601` and `adyenService.ts:265`, each setting `due_date: period.dueDate`. The derivation is covered by the anniversary, clamping and year-boundary checks in `npm run check:billing`. Bills are raised on demand rather than by a scheduled job, which is correct for this property: collection happens in person, and a nightly generator would raise bills for tenants the owner has already been paid by. "Overdue" is derived on read for the same reason (BR-011). | **Enforced** |
...
| **BR-036** | Water Payment Validation | Water Payment Validation | **CORRECTED 2026-09-14.** The original evidence said no comparison exists. Two do, in the on-site payment modal and the income-ledger edit: a figure below the occupant baseline is refused, and one that is not a whole multiple of the rate is refused — both before saving, both naming the expected figure, which is what the rule asks for. A related defect **was** found and fixed on the same date: the modal hardcoded the rate as `200` in six places and the Linda charges as `400`/`200`, so the landlady could change her rate and the form would go on validating against the old one. Both now come from `GET /api/public/rates`, which reads `system_settings`. **Partial**, not Enforced: the check is client-side, so a direct API request bypasses it. **CLOSED 2026-09-14, one layer later.** The comparisons existed but the income-ledger form - the one the owner uses to correct the ledger - still validated against a **hardcoded** `occupants * 200`, with `LF` and `LB` pinned at 400 and 200 and the figure required to be a whole multiple of 200. `OnsitePaymentModal` had been moved onto `GET /api/public/rates`; this form had not, so the moment the rate changed in settings the two layers disagreed and this one would reject a correct entry. It now reads the same endpoint, with the seeded values as the fallback so a failed fetch degrades to the old behaviour rather than to no validation. A second defect went with it: the "whole multiple" rule was applied to **LF** and **LB** as well, which are on fixed charges (BR-040) and are not multiples of anything - it passed only because 400 and 200 happen to divide by 200, and would have started refusing them the moment either rate changed. | **Enforced** |
...
| Status | Count | Rules |
| --- | --- | --- |
| **Enforced** | 40 | BR-001, BR-002, BR-003, BR-004, BR-005, BR-006, BR-007, BR-008, BR-009, BR-010, BR-011, BR-012, BR-013, BR-014, BR-015, BR-016, BR-017, BR-018, BR-019, BR-021, BR-022, BR-023, BR-024, BR-026, BR-027, BR-028, BR-030, BR-031, BR-032, BR-033, BR-035, BR-036, BR-038, BR-041, BR-042, BR-043, BR-044, BR-045, BR-047, BR-048 |
| **Partial** | 6 | BR-020, BR-025, BR-029, BR-034, BR-039, BR-049 |
