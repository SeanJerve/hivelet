...
| **G-7** | **Two payment endpoints are unauthenticated.** Neither carries `requireAuth`, `optionalAuth`, nor `requirePermission` | `public.ts:202` GET `/public/payments/mock-gateway`; `public.ts:853` POST `/public/payments/mock-gateway/complete` | 3.0, 7.0 | **PARTIAL** — Phase 3 hardening item |
