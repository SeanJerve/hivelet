...
**The measured reason this column is necessary.** Counting `.from('...')` database invocations across `backend/src/routes/` and `backend/src/services/` yields **164** calls, of which **131 (80%)** sit inside route handlers: 108 in `backend/src/routes/admin.ts`, 17 in `backend/src/routes/tenant.ts`, 6 in `backend/src/routes/public.ts`. `backend/src/routes/admin.ts` is **2,263 lines**. Phase 3 is a mechanical extraction of those handlers into the eight planned services; it is not a redesign, which is why the process-to-component mapping below is stable across the extraction.
...
| :--- | :--- | :--- | :--- | :--- |
| **G-1** | ~~D11 System Parameters has no owner in code.~~ **CLOSED 2026-09-13** | `services/settingsService.ts` owns D11; `billingService.ts` consumes it; `GET /api/public/rates` exposes the quotable part | D11, `settingsService.ts` | **RESOLVED** — the water rate is read from `system_settings.water_rate_per_occupant` (BR-014, BR-036) and `grace_period_days` is **0** per OD-16 and migration `016` |
...
| **G-4** | ~~`fifty_percent_share` and `remitted_amount` are computed but never written.~~ **WITHDRAWN — this was never a defect.** | **CORRECTED 2026-09-13:** both columns are `GENERATED ALWAYS AS ... STORED` (`is_generated = 'ALWAYS'` in `information_schema.columns`), so PostgreSQL derives them and rejects any write naming them. Their absence from the INSERT is required. All 937 live rows hold correct values. | D7, 3.0, 6.0 | **WITHDRAWN** — BR-035 and BR-038 are satisfied by database-enforced generation |
