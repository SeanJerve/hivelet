...
These thirteen are not FR failures and are therefore not counted in the 26 / 14 / 3 / 1 totals of §3. They are carried
...
The 14 PARTIAL and 3 MISSING rows are equally part of the honest account. **Two of the three this paragraph named are now delivered**, and it is corrected rather than quietly dropped: the system now *identifies* overdue and grace-period status, which is what FR-013 asks for — `isOverdue()` is called by both bill endpoints and surfaced as `effective_status` (FR-013, closed 2026-09-14). **It does not yet tell Mrs. Fe, and an earlier draft of this sentence said it did.** Nothing in `frontend/src` calls `GET /admin/bills`: the administrator has no bills screen, so the overlay is computed on her side and never asked for. The tenant sees their own. FR-013 stands as MAPPED on the requirement as written — *“identify overdue payments and grace-period status”* — and the missing administrator surface is recorded in §5.1 rather than hidden inside that status, and it *does* bill LF and LB by the rule she actually applies (FR-036, `computeWaterFee()` over the seeded `linda_*` settings). What remains true is the third and largest: **the system still does not compute its own net position** (FR-019), nor its profitability trend (FR-020), and it does not remember an occupant count from one month to the next (FR-033). Those three are the Phase 3 sequence, and FR-019 is the one that most directly touches her daily routine.
