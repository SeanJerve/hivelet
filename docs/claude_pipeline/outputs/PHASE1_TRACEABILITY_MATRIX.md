...
**FR-009 — disclosed defect, CLOSED 2026-09-22.** Onboarding used to assign a shared literal temporary password, `'Hivelet@Tenant2026'`, hashed with bcrypt cost 12. It now generates a random one per tenant — `backend/src/utils/generateTemporaryPassword.ts`, `crypto.randomInt` rather than `Math.random()`, over a character set that omits the ambiguous pairs (0/O, 1/l/I) because the figure is read aloud once and typed once. The administrator is shown it exactly once, in a non-dismissible modal, and it is never logged or stored in plaintext; the bcrypt hash also stopped being echoed in the onboarding response, which it had been via `.select('*')`. Forcing a replacement at first login is written and wired but deliberately inert until `database/migrations/048_must_change_password.sql` is applied — see **B-53** in `BLOCKED_FOR_SEAN.md`. The record-management behaviour the requirement asks for was always fully present; this was a credential-issuance weakness, not an FR-009 gap.
...
> | **FR-040** Cumulative Totals | year-to-date view is lost | **Stale as of 2026-09-26 — no longer Partial, now Done.** The gating decision this row was waiting on, **OD-07**, closed 2026-09-17: the category cumulative is a computed window that restarts each January, not a stored column. `expenseReportExport.ts` computes both the "this month" total and the running cumulative per category (`cumulative` map, lines 210-280) and `npm run check:reports` asserts the whole chain against every month of 2024-2026 plus the year-end figures. **BR-046** is now Enforced by that computation, not "Not enforced" as this row still said. |
...
>
> So the honest count as of 2026-09-15 was **three missing, two partial, five done** — not ten
> missing. **Re-verified 2026-09-26: FR-040 moved from Partial to Done** once OD-07 closed on
> 2026-09-17 (see its row above, corrected in this pass). The count today is **three missing, one
> partial, six done**.
>
...
**The item this used to flag as latent, A-14, is now closed** — migration `022` made
`public.current_user_role()` return `NULL` rather than `'admin'` when it cannot identify the
caller, exactly the fix this paragraph called for. See A-14's row in §5.1, corrected 2026-09-26.
...
| A-3 | ~~**50% share divisor hardcoded** as `rentAmount / 2` at two call sites rather than derived once~~ — **WITHDRAWN 2026-09-22. The finding was stale and its remedy was impossible.** Three things, each checked against the live catalogue or the current source rather than against a document: (1) **the call sites are gone** — `admin.ts` contains no half-of-rent arithmetic anywhere, and the only `rent_amount / 2` left in the backend is a fallback in `incomeReportExport.ts` that is dead, because the column it falls back from is never null; (2) **the remedy could never have been implemented** — `fifty_percent_share` is `GENERATED ALWAYS AS (rent_amount / 2.0)`, read from `information_schema.columns`, so PostgreSQL computes it on every write and rejects any INSERT or UPDATE that supplies a value for it; application code cannot write it from a setting or from anything else; (3) **its evidence was `FULL_DATABASE_SCHEMA.sql`**, the file `CLAUDE.md` rule 2 says does not describe this database and has been wrong about it repeatedly — this row is another instance of exactly that, and the crosswalk's BR-035 entry had already reached the correct conclusion from `information_schema`. Keep the figure described only as a system-computed value equal to half that row's Rent Amount, retained for ledger parity with the historical spreadsheet. | — | **Closed** | Nothing to do. `settingsService.getRevenueSharePercent` already documents that the setting does not drive the stored figure. |
...
| ~~**A-12**~~ | ~~`POST /api/auth/change-password` is called by nothing, and no password-change UI exists.~~ **CLOSED 2026-09-17.** `ChangePasswordModal.vue` is offered from the account menu to **both** roles; the administrator was onboarded on a literal too. `check:endpoints` caught its own stale exemption on the first run after wiring. Six assertions in `check:api` cover the failure paths - wrong current password (**401 `INVALID_CREDENTIALS`**, deliberately not treated as a session failure), the three password-strength rules, a missing current password, and no token. The success path is not asserted: it would rotate a live credential the rest of the suite depends on. | Zero matches for `change-password`, `newPassword` or `currentPassword` in `frontend/src`, verified 2026-09-16 | **Phase 3 — highest of these rows** | Add a change-password form for both roles against the existing endpoint. Until then every resident account shares one password that cannot be changed from the product. |
| **A-13** | ~~Five endpoints are superseded rather than unplugged~~ **CLOSED 2026-09-17, commit `7dfa2f6` "retire the five superseded endpoints, and NOT the unplugged one".** All five — `PATCH /admin/tenants/:profileId/status`, `PATCH /admin/tickets/:ticketId/close`, `GET /public/clusters`, `GET /public/rooms/:roomId`, `POST /admin/rooms/:roomId/photo` — are gone from `backend/src/routes/admin.ts` and `public.ts`; none of the five route patterns matches anywhere in either file today. The commit message itself distinguishes this row from A-11, which it deliberately left alone. | Route census against `backend/src/routes/admin.ts`, `public.ts`, re-verified 2026-09-26 | **Closed** | Nothing further to do. |

| **A-14** | ~~`public.current_user_role()` is `SECURITY DEFINER` and fails OPEN to `admin`.~~ **CLOSED — migration `022_current_user_role_fails_closed.sql`, explicitly filed as "Remediation row A-14" in its own header.** The function now returns `NULL` when it cannot identify the caller, exactly the remediation this row proposed. Read in full 2026-09-26: the migration's own rationale matches this row's — `NULL = 'admin'` and `NULL <> 'admin'` are both `NULL`, so a `USING` clause built on it denies rather than defaulting to admin. `auth_user_id` is left unpopulated deliberately, as this row also recommended. | `database/migrations/022_current_user_role_fails_closed.sql` | **Closed** | Nothing further to do. |
...
| **A-19** | **A unit photo can be added but never removed, and clearing one looks like it worked.** `PATCH /admin/rooms/:roomId` guards the photo write with `if (photo && photo.trim().length > 0)`, so an empty string is ignored and the existing `room_photos` row survives — there is no delete path anywhere for a room photo. `AdminEditUnitModal` nevertheless sets `unit.value.photo = ''` in local state, so **the interface shows the photo cleared and a reload brings it back**. The same shape as the tenant profile form that discarded a name edit while updating the header. **Latent, not live:** `room_photos` holds **0 rows** across all 33 units, so nobody has ever had a photo to clear. | Route handler read 2026-09-16; `room_photos` row count from the live database | **Phase 3** | Either delete the unit's photo rows when an empty string is submitted, or stop the modal from clearing its local copy. The first is what the interface currently promises. |
