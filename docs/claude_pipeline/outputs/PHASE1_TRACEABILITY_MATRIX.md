...
| FR-015 | Online Payments | Tier 5 / Tier 2 | `adyenService.ts` `[I]` | `POST /api/tenant/payments/checkout` (`tenant.ts:379`); `POST /api/tenant/payments/adyen/verify-session` (`tenant.ts:514`); `GET /api/public/payments/mock-gateway` (`public.ts:201`); `POST /api/public/payments/mock-gateway/complete` (`public.ts:852`) | `bills`, `payments`, `room_assignments`, `rooms`, `profiles`, `notifications` | **PARTIAL** |
...
| A-8 | **Two payment endpoints are unauthenticated** | `GET /api/public/payments/mock-gateway` (`public.ts:201`); `POST /api/public/payments/mock-gateway/complete` (`public.ts:852`) | **Phase 3** | Place both gateway-return endpoints behind an authenticated or signature-verified check; a hardening item, disclosed rather than discovered. |
