...
**FR-013 — why MISSING.** The literal `'Overdue'` occurs exactly once in `backend/src`, as a filter value in an `IN` clause at `admin.ts:1143`. No handler, no scheduled job and no database trigger ever transitions a bill from `Due` to `Overdue`. The one line that wrote `grace_period_end_date` hardcoded a **10-day** window, contradicting both BR-012 and the seeded `grace_period_days = 7`. **CLOSED 2026-09-13:** `billingService.computeBillPeriod()` reads the setting, and migration `016` set it to **0** per OD-16, so `grace_period_end_date` equals `due_date`. *(The absence of an Overdue transition is still open.)* `GET /api/tenant/my-bills` (`tenant.ts:75`) faithfully returns a column that nothing maintains.
