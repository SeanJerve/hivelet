...
/**
 * @deprecated Superseded by the authenticated session in `lib/authStore.ts`.
 *
 * This was a client-assignable role flag: any visitor could set it to 'admin'
 * from the navbar and the entire management UI would render. System Bible
 * Section 20 requires the role to come from an authenticated identity, so read
 * `currentRole` / `isAdmin` / `isTenant` from the auth store instead.
 *
 * Retained only so any not-yet-migrated view keeps compiling. Do not read it
 * for authorization, and do not assign to it.
 */
...
export const isInquiryModalOpen = ref(false);
export const inquiryPreselectedUnit = ref<string | null>(null);

export function openInquiryModal(unitCode?: string) {
  inquiryPreselectedUnit.value = unitCode ?? null;
